...
static DebugLogLevel s_DebugLogLevel = DebugLogLevel::kHighAssert;
  ...
  switch (severity) {
    case GL_DEBUG_SEVERITY_HIGH:
      if ((int)s_DebugLogLevel > 0) {
        ...
        if (s_DebugLogLevel == DebugLogLevel::kHighAssert)
          GLCORE_ASSERT(false, "GL_DEBUG_SEVERITY_HIGH");
      }
      break;
    case GL_DEBUG_SEVERITY_MEDIUM:
      if ((int)s_DebugLogLevel > 2) LOG_WARN("[OpenGL Debug MEDIUM] {0}", message);
      break;
    case GL_DEBUG_SEVERITY_LOW:
      if ((int)s_DebugLogLevel > 3) LOG_INFO("[OpenGL Debug LOW] {0}", message);
      break;
    case GL_DEBUG_SEVERITY_NOTIFICATION:
      if ((int)s_DebugLogLevel > 4) LOG_TRACE("[OpenGL Debug NOTIFICATION] {0}", message);
      break;
  }
}
